    ...
    <Markdown
      ...
      components={{
        h1: ({ className, node, ...props }) => (
          <h1
            className={cn("text-3xl font-bold mt-8 mb-6", className)}
            {...(props as any)}
          />
        ),
        ...
        h3: ({ className, node, ...props }) => (
          <h3
            className={cn("text-xl font-bold mt-4 mb-3", className)}
            {...(props as any)}
          />
        ),
        ...
        p: ({ className, node, ...props }) => (
          <p className={cn("", className)} {...(props as any)} />
        ),
        ul: ({ className, node, ...props }) => (
          <ul className={cn("list-disc pl-8", className)} {...(props as any)} />
        ),
        ol: ({ className, node, ...props }) => (
          <ol
            className={cn("list-decimal pl-8", className)}
            {...(props as any)}
          />
        ),
        ...
        blockquote: ({ className, node, ...props }) => (
          <blockquote
            className={cn(
              "border-l-4 border-accent pl-4 my-4 text-accent-foreground",
              className
            )}
            {...(props as any)}
          />
        ...
        hr: ({ className, node, ...props }) => (
          <hr
            className={cn("my-6 border-t border-accent", className)}
            {...(props as any)}
          />
        ),
        ...
        th: ({ className, node, ...props }) => (
          <th
            className={cn(
              "border border-accent px-4 py-2 bg-accent font-bold",
              className
            )}
            {...(props as any)}
          />
        ...
        td: ({ className, node, ...props }) => (
          <td
            className={cn("border border-accent px-4 py-2", className)}
            {...(props as any)}
          />
        ),
